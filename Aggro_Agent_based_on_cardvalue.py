...
# This agent would choose cards of highest value and summon minion to attack opponent as quick as possible to defeat opponent.
...
while True:
    ...
    print_str = ""
    # In draft stage agent choose cards with highest card value
    if player_state[1] == 0:
    ...
    # Attack opponent after wipe out all creatures with 'Guard' ability in opponent's board
            ...
            for i in range(len(player_hand)):
                ...
                if player_hand[value_seq[i]][4] < player_state[1]:
                    if player_hand[value_seq[i]][3] == 0:
                        ...
                        # Record those creatures summoned in this round with 'Charge' ability
        ...
        # Try to summon minion again after attack round in case there are already six minions in first round
        if len(player_hand) > 0 and len(player_board) < 6:
            value_seq = sorted(player_hand.keys())
            for i in range(len(player_hand)):
                if player_state[1] <= 0:
