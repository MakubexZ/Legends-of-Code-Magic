...
# Aggro agent for dealing with the Bronze League has the ability to handle more complex situation
# This agent would choose cards of highest value under certain conditions
# Summon minion and use items to defeat opponent as quick as possible.
...
# Set up pefer cost curve of deck by limit the numbers of cards in different cost and types
...
while True:
    ...
    # Read each input of all information of this game state
    # Aggro agent has to understand the situation of this state
    for i in range(2):
    ...
    for i in range(card_count):
        ...
        # Record different card information of draft stage and battle stage
        # For draft stage we only care about card value
        # For battle stage we record all card information by a hash table with card value as the key
        if player_state[1] == 0:
    ...
    print_str = ""
    # In draft stage agent choose cards with highest weighted card value
    # Cards value is weighted by Weighted by the difference between picked number and perfer number according to a certain rule
    if player_state[1] == 0:
        print("Draft", file=sys.stderr)
        pick_percent = []
        # Calculate the difference between picked number and perfer number of this COST
        for i in range(3):
        ...
        # Adjust card value by moderately increasing or decreasing this value
        for i in range(3):
            if pick_percent[i] > 0:
                pick_percent[i] = (1 + pick_percent[i]/50) * card_draft_list[str(card_draft_id[i])][0]
        # Choose the card with highest pick percentage
        pick_number = pick_percent.index(max(pick_percent))
        print_str += "PICK " + str(pick_number)

        # Record the COST of this card agent chose
    ...
    # In battle stage agent observe the board of opponent and attack those creatures with 'Guard' ability
    # Attack opponent after wipe out all creatures with 'Guard' ability in opponent's board        
    else:
        ...
        # Sort creatures with 'Guard' ability in ascending order of HEALTH POINT
        op_G_seq = sorted(op_G.values())
        
        att_seq = []
        # Sort summoned minion in own board in ascending order of ATTACK
        for value in player_board.values():
            bisect.insort(att_seq, [value[5], value[1]])
            
        # Using card as much as possible in descending order of card value
        if len(player_hand) > 0 and len(player_board) < 6:
            value_seq = sorted(player_hand.keys())
            value_seq.reverse()
            used = []

            # Deal with creatures, green item, red item and blue item in different way
            for i in range(len(player_hand)):
                ...
                # If this card is used, subtract the COST from the remaining mana
                if use_card == 1:
                    player_state[1] -= player_hand[value_seq[i]][4]

            # Delete used cards in this round
            for i in range(len(used)):
                del player_hand[value_seq[used[i]]]

                
        # Try to use card again before attack round
        # In case there is no minion before first round and could use items this round
        if len(player_hand) > 0 and len(player_board) < 6:
            value_seq = sorted(player_hand.keys())
            value_seq.reverse()
            used = []
        ...
        # Attack creatures with 'Guard' ability in opponent's board if they exist
        # Attack opponent if not
        atted = []
        ...
        # Try to use card again in the last round in case there are still remaining mana
        if len(player_hand) > 0 and len(player_board) < 6:
            value_seq = sorted(player_hand.keys())
            value_seq.reverse()
            used = []
